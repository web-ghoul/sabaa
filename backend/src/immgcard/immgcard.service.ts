...
export class ImmgcardService {
  constructor(
    @InjectModel(IMMGCard.name) private immgcardModel: Model<IMMGCard>,
  ) {}
  ...
  async getCounters(cardType: string) {
    const count = await this.immgcardModel.estimatedDocumentCount({
      cardType: cardType,
    });
    return {
      count,
