...
import { CustomersSheetTypes } from "../types/contexts.types";

const UploadCustomers = () => {
  const { customersSheets } = useContext(ExcelsContext);
    ...
    <PrimaryBox>
      <PrimaryContainer className={pageContainerClasses}>
        ...
        <Box className={`grid justify-stretch items-start gap-8`}>
          <UploadExcel variant={"customers"} />
          {customersSheets.length > 0 ? (
            customersSheets.map((sheet: CustomersSheetTypes, i) => (
              <SheetAccordion title={sheet.fileName} key={i}>
                <CustomersTable
                  ...
                  fileIndex={i}
                />
                <Forms type={"createCustomersSheet"} index={i} />
              </SheetAccordion>
            ))
          ) : (
            <NoSheetsFound />
